...
class CarObject {
	...
	update() {
		...
		if (!(this instanceof MyCar) && this.y >= 100) this.remove();
	}
...
class MyCar extends CarObject {
	...
	crash() {
		...
		if (gameManager.lives == 0){
			...
			window.setTimeout(() => this.reset(), 1500);
		}
	}
}

class ObstacleCars extends CarObject {
	constructor(x, width, height, vehicle, speed, dom){
...
class GameManager {
	...
	generateObstacles() {
		...
		const carSpeed = 0.5 + 0.0005 * this.tickCount;
		this.objects.add(new ObstacleCars(lane, 10, 20, vehicle, carSpeed, newDom));
		...
		setTimeout(() => this.generateObstacles(), 1500 * (2000)/(2000 + this.tickCount));
	}
